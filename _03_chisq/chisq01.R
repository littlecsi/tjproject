...
getMonthlyCmt <- function(df) {
    ...
    for(l in c(1:len)) {
        month <- df[l,4] %>% str_sub(6, 7)
        # cat(month, "\n")
        if(month == "01") { x1 <- c(x1, df[l,6]) }
        if(month == "02") { x2 <- c(x2, df[l,6]) }
        if(month == "03") { x3 <- c(x3, df[l,6]) }
        if(month == "04") { x4 <- c(x4, df[l,6]) }
        if(month == "05") { x5 <- c(x5, df[l,6]) }
        if(month == "06") { x6 <- c(x6, df[l,6]) }
        if(month == "07") { x7 <- c(x7, df[l,6]) }
        if(month == "08") { x8 <- c(x8, df[l,6]) }
        if(month == "09") { x9 <- c(x9, df[l,6]) }
        if(month == "10") { x10 <- c(x10, df[l,6]) }
        if(month == "11") { x11 <- c(x11, df[l,6]) }
        if(month == "12") { x12 <- c(x12, df[l,6]) }
    }
...
chisq.test(x=PmCmtTotal, y=SmCmtTotal)
...
chisq.test(x=EmCmtTotal, y=WmCmtTotal)
# 	F test to compare two variances
# 
